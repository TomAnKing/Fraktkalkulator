...
const App: React.FC = () => {
  ...
  const { 
    rawTotalLoadingMeters, 
    billableLoadingMeters, 
    totalCost, 
    loadingRampSurcharge, 
    subtotal,
    isUsingPallets,
    palletCount 
  } = useMemo(() => {
    ...
    let usingPallets = false;
    let pCount = 0;

    // Special rule for Oslo: under 0.8 LDM uses pallets (0.4 LDM per pallet, 630 kr per pallet)
    if (destination === 'Oslo' && rawTotal > 0 && rawTotal < 0.8) {
      usingPallets = true;
      pCount = Math.ceil(rawTotal / 0.4);
      currentSubtotal = pCount * 630;
      billableTotal = pCount * 0.4;
    } else {
  ...
  const generatePdf = () => {
    ...
    if (isUsingPallets) {
      doc.text(`Fakturerbare paller:`, 20, currentY);
      doc.text(`${palletCount}`, 190, currentY, { align: 'right' });
      currentY += 7;
    } else {
    ...
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 lg:p-8 bg-slate-100">
      <div className="w-full max-w-4xl bg-white p-6 sm:p-8 rounded-2xl shadow-2xl space-y-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-slate-500">Fraktkalkulator beregner ikke ut ifra volum og tar derfor ikke hensyn til at varer kan stables på pallene. Dersom det skal sendes større volum av varer som kan stables kan en halvere den estimerte fraktkostnaden.</p>
        </div>
